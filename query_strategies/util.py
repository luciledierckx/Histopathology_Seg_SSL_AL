...
def get_unique_folder(root: pathlib.Path, prefix: str):
    "return a folder as 'root/prefix-now(-idx)?' that is guaranteed to be unique"
    ...
    for suffix in _suffix():
        directory = root / (prefix + "-" + now + suffix)
        try:
        ...
        assert False, "this is a bug, _suffix() should be infinite"
